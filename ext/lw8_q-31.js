...
(function (global) {
  ...
  function _lw8() {
    try {
      if (sessionStorage.getItem('8n-refresh-page') == 1) {
        ...
        if (year > 2018 && document.getElementById('copyright-year')) {
          document.getElementById('copyright-year').innerHTML = year;
        }
        window &&
          window.lw8Options &&
          window.lw8Options.applicationServerKey &&
  ...
  _lw8.prototype.ajaxJSON = function (method, url, obj, success, error, complete, quiet) {
    ...
    xHTTP.send(
      method.toLowerCase() === 'get' ? '' : obj instanceof FormData ? obj : JSON.stringify(obj)
    );
  };
  ...
  _lw8.prototype.getElementColor = (e, prop) => {
    ...
    return (
      ...
      ('0' + parseInt(color[3]).toString(16)).slice(-2)
    );
  };
  ...
  _lw8.prototype.getPrefixedName = function (prefix, name) {
    return prefix + name.charAt(0).toUpperCase() + name.slice(1);
  };

  _lw8.prototype.getURLParams = function () {
  ...
  _lw8.prototype.hideDialog = function (e, show) {
    ...
    while (
      ...
      !e.classList.contains('slideover') &&
      !e.classList.contains('subscribe') &&
      e !== document.body
    ) {
      e = e.parentNode;
    }
  ...
  _lw8.prototype.isNumeric = function (i) {
    return !isNaN(i - parseFloat(i));
  };
  ...
  _lw8.prototype.removeExpiredElements = function (e) {
    ...
    if (expiringElements.length === 0 && e !== document && e.classList.contains('remove-after')) {
      expiringElements = [e];
    }
    for (var i = 0; i < expiringElements.length; i++) {
      this.simpleUTCDateTimeParse(expiringElements[i].getAttribute('data-after-value')) <
        Date.now() && expiringElements[i].remove();
    }
    expiringElements = e.querySelectorAll('.remove-before');
    if (expiringElements.length === 0 && e !== document && e.classList.contains('remove-before')) {
      expiringElements = [e];
    }
    for (i = 0; i < expiringElements.length; i++) {
      this.simpleUTCDateTimeParse(expiringElements[i].getAttribute('data-before-value')) >
        Date.now() && expiringElements[i].remove();
    }
  };
  ...
  _lw8.prototype.scrollPeek = function () {
    ...
    if (scrollTop > document.body.scrollHeight - document.body.offsetHeight - 100) {
      ...
      if (scrollTop > this.lastScroll) {
        this.navElem &&
          (this.navElem.classList.add('nav-hide'),
          this.notificationsElem.classList.add('nav-hide'));
      } else {
        this.navElem &&
          (this.navElem.classList.remove('nav-hide'),
          this.notificationsElem.classList.remove('nav-hide'));
      }
    } else {
      this.scrollToTopElem && this.scrollToTopElem.classList.add('hide');
      this.navElem &&
    ...
    if (scrollTop > 600) {
      for (var i = 0; i < this.subscribeElems.length; i++) {
        var subscribedStatus = this.getCookie(
          'subscribed' + this.subscribeElems[i].getAttribute('data-campaign-name')
        );
        if (
          !(subscribedStatus == 1 || subscribedStatus == 2) &&
          this.subscribeElems[i].getAttribute('data-shown') !== 'true'
        ) {
  ...
  _lw8.prototype.setupAjaxHTMLs = function (e) {
    ...
    if (htmls.length === 0 && e !== document && e.classList.contains('ajax-html')) {
      htmls = [e];
    }
    htmls.forEach((html) => {
  ...
  _lw8.prototype.setupLinks = function (e) {
    ...
    if (links.length === 0 && e !== document && e.classList.contains('ajax-href')) {
      links = [e];
    }
    for (var i = 0; i < links.length; i++) {
      links[i].addEventListener('click', function () {
        lw8.ajaxJSON(this.getAttribute('data-method'), this.getAttribute('data-href'), {
  ...
  _lw8.prototype.setupLocalData = function () {
      ...
      Object.keys(localData).forEach((key) => {
        if (localData[key]) {
          localStorage.setItem(
            key,
            Object.prototype.toString.call(localData[key]) === '[object Object]'
    ...
    window &&
      window.lw8Options &&
      window.lw8Options.localData &&
      Object.keys(window.lw8Options.localData).forEach((key) => {
        if (window.lw8Options.localData[key]) {
          localStorage.setItem(
            key,
            Object.prototype.toString.call(window.lw8Options.localData[key]) === '[object Object]'
              ? JSON.stringify(window.lw8Options.localData[key])
              : window.lw8Options.localData[key]
          );
        } else {
          localStorage.removeItem(key);
        }
  ...
  _lw8.prototype.setupNotifications = function () {
    if (!navigator.cookieEnabled) {
      this.addNotification(
        'error',
        'Our site works better with Cookies. Please enable them in your browser.'
      );
    }
    //this is necessary for 301/302 redirects
  ...
  _lw8.prototype.setupPopups = function (e) {
    ...
    if (popups.length === 0 && e !== document && e.classList.contains('popup')) {
      popups = [e];
    }
    for (var i = 0; i < popups.length; i++) {
      ...
      !popups[i].querySelector('form') &&
        popups[i].addEventListener('click', function (ev) {
          ev.target.classList.contains('popup') && lw8.hideDialog(this);
        });
      var close = document.createElement('div');
      close.classList.add('close');
      ...
      popups[i].firstElementChild.firstElementChild.insertBefore(
        close,
        popups[i].firstElementChild.firstElementChild.firstElementChild
      );
    }

    var showPopupElems = e.getElementsByClassName('show-popup');
    if (showPopupElems.length === 0 && e !== document && e.classList.contains('show-popup')) {
      showPopupElems = [e];
    }
    for (i = 0; i < showPopupElems.length; i++) {
      showPopupElems[i].addEventListener('click', function () {
        ...
        var iframe = document
          ...
          .querySelector('iframe.play-video');
        if (iframe) {
          var requests = [
            'requestFullscreen',
          ...
          'onfullscreenchange' in iframe &&
            iframe.addEventListener('fullscreenchange', function () {
              !document.fullscreenElement && lw8.hideDialog(this);
            });
          'onwebkitfullscreenchange' in iframe &&
            iframe.addEventListener('webkitfullscreenchange', function () {
              !document.webkitFullscreenElement && lw8.hideDialog(this);
            });
          iframe.contentWindow.postMessage('{"event":"command","func":"playVideo","args":""}', '*');
        }
        var video = document
          .getElementById(this.getAttribute('data-target'))
          .querySelector('video.play-video');
        video && video.play();
      });
    }
    ...
    e.addEventListener('click', () => {
      Notification.requestPermission((result) => {
        result === 'granted' &&
          ...
          window.serviceWorkerRegistration.pushManager.getSubscription().then((subscription) => {
            !subscription &&
              window.serviceWorkerRegistration.pushManager
                .subscribe({
                  applicationServerKey: lw8.base64ToArray(window.lw8Options.applicationServerKey),
  ...
  _lw8.prototype.setupSession = function () {
    ...
    if (loggedIn) {
      if (profile) {
        ...
        if (params && params.has('asUserId')) {
          ...
          profileMenu.innerHTML =
            '<li><a href="/account?asUserId=' +
            params.get('asUserId') +
            '">As User</a></li>' +
            '<li><a href="/account">Back to ' +
            data.name +
            '</a></li>';
        } else {
      ...
      if (profile) {
        ...
        profileMenu.innerHTML =
          '<li><a class="show-popup" data-target="login">Login</a></li>' +
          '<li><a class="show-popup" data-target="signup">Sign Up</a></li>';
        this.setupPopups(profileMenu);
      }
      if (blogCommentPost) {
        blogCommentPost.innerHTML =
          '<p>Add you voice to this conversation.</p>' +
    ...
    for (var i = 0; i < slideovers.length; i++) {
      if (slideovers[i].classList.contains('minimizable')) {
        ...
        minimize.addEventListener('click', function () {
          this.parentNode.parentNode.classList.toggle('minimized');
        });
        slideovers[i].firstElementChild.insertBefore(
          minimize,
          slideovers[i].firstElementChild.firstElementChild
        );
      }
      var close = document.createElement('div');
      close.classList.add('close');
  ...
  _lw8.prototype.setupSubscribes = function () {
    for (var i = 0; i < this.subscribeElems.length; i++) {
      ...
      if (
        (subscribedStatus == 1 || subscribedStatus == 2) &&
        this.subscribeElems[i].classList.contains('remove')
      ) {
        ...
        close.addEventListener('click', function () {
          var e = this;
          while (!e.classList.contains('subscribe')) {
            e = e.parentNode;
          }
          lw8.setCookie('subscribed' + e.getAttribute('data-campaign-name'), 1, 72);
          for (var i = 0; i < lw8.subscribeElems.length; i++) {
            lw8.subscribeElems[i].getAttribute('data-campaign-name') ===
              e.getAttribute('data-campaign-name') && lw8.hideDialog(lw8.subscribeElems[i]);
          }
          lw8.adjustHeaderHeight();
        });
      }
